...
def get_list_of_input_images(filepath, excluded_extensions):
    return [
        ...
        if file.split(".")[-1] not in excluded_extensions and not os.path.isdir(file)
    ]
...
if __name__ == "__main__":
    ...
    filelist = get_list_of_input_images(
        filepath=args.img_path, excluded_extensions=args.excluded_extensions
    )
    for filename in tqdm(filelist):
        img = Image.open(filename)
        new_image = resize_image(img, scale_factor=args.scale_factor)
        final_output_name = get_final_output_name(
            filename, output_dir=args.output_dir, output_extension=output_extension
        )
